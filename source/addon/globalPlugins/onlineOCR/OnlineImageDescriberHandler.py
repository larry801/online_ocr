...
def safeJoin(a, b):
    """
    join paths safely without unicode error
    @param a:
    @type a: str
    @param b:
    @type b: unicode
...
class BaseDescriber(ContentRecognizer, AbstractEngine):
    """
    Abstract base BaseDescriber
    """
    ...
    def CopyToClipboardSetting(cls):
        return BaseDescriber.BooleanSetting(
            "clipboard",
    ...
    def form_encode(options):
        ...
        if config.conf["onlineImageDescriber"]["verboseDebugLogging"]:
            log.io(payload)
        return payload
...
class OnlineImageDescriberPanel(AbstractEngineSettingsPanel):
    ...
    def onSave(self):
        super(OnlineImageDescriberPanel, self).onSave()
